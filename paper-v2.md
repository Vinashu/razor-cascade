...
Judge agreement—defined as the fraction of step scores within 1 point across repeats—exceeded 98.7% across all configurations (range: 98.7% for baseline-gemini to 99.4% for grok and gemini cascades). This validates the heuristic as a reasonable proxy for quality in this study.

### 3.6 Runs and Statistical Design

Each of the seven configurations was run 10 times with live API calls. This gives us 70 runs total and 700 individual step executions (up from 32 runs / 320 steps in the preliminary study).
...
The high judge agreement scores (98.7–99.9%) across all configurations confirm that our heuristic scoring rubric is a reasonable proxy for quality. This is a useful result for the community: it suggests that well-calibrated heuristic scores can stand in for judge-based evaluation in settings where running a judge model adds cost and complexity.

We note that the judge occasionally failed to parse responses (see log entries for task 3 in some runs), defaulting to heuristic scores. These failures were rare and did not affect aggregate results.
...
**Quality ceiling effect.** Mean quality scores above 9.0 for OpenAI and xAI suggest a ceiling effect in the heuristic rubric. Real quality differences may be masked.
...
The OpenAI cascade with gpt-5-nano as gate achieved the deepest cost cut (49.8%), the xAI cascade with grok-code-fast followed closely (48.4%), and the Gemini cascade with gemini-2.5-flash showed only a non-significant 8.1% reduction. All 70 cascade runs passed every test. Quality was retained at 96–97% of baseline for the effective cascades, and LLM judge validation confirmed that heuristic scoring aligned with independent evaluation (agreement > 98.7%).
