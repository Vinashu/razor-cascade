...
The `--judge-repeat 2` setting produced two judge attempts for each flagship step. The study runner stores per-step judge score standard deviation and a derived `mean_judge_agreement` proxy, but the stricter within-1-point agreement rates reported here were computed post hoc from the archived repeated judge snapshots. Across 700 flagship steps, 698 produced two parseable judge scores. Among those 698 pairs, 697 (99.9%) were within 1 point and 641 (91.8%) were identical; if the two baseline-gemini steps with a missing parseable repeat are counted in the denominator, the within-1-point rate is 99.6% over all 700 flagship steps. Per-configuration within-1-point rates across all 100 steps ranged from 98% (baseline-gemini) to 100% (baseline-openai, openai-mini, baseline-grok, grok, and gemini), with openai-nano at 99%, confirming that claude-haiku-4-5 was a highly consistent scoring instrument for this rubric.

Only two steps exceeded the pipeline's warning threshold of `judgeScoreStddev > 1.5`: both were `baseline-gemini` step 3 ("List, filter, and view tasks") in runs 5 and 9, each recording a final `qualityScore` of 3.2 and a stddev of ~4.53. In both cases the flagship response itself was empty. One judge repeat returned a parseable score of 0, while the other returned explanatory text without a parseable score, causing that repeat to fall back to the heuristic scorer. These same artifact failures are discussed further in Section 5.9.
...
**Judge-response artifact.** Two `baseline-gemini` runs (run 5 and run 9, task 3) produced an empty flagship response, which then propagated into both judge prompts as an empty candidate response. In each case one judge repeat returned a parseable score of 0, while the other returned explanatory text without a parseable score, causing heuristic fallback for that repeat. Only 2 of 700 flagship steps were affected, so the aggregate impact is negligible, but the empty-response failure should be investigated before future studies.
